...
//	$dbGroupsID = DBselect('SELECT groupid FROM '.$grps.' WHERE groupid <> 1 AND name IN ("'.$groupsini.'") ORDER BY name ASC');
//	$dbGroups = DBselect( 'SELECT * FROM '.$grps.' WHERE groupid <> 1 ORDER BY name ASC');
//	$teste = 'SELECT groupid FROM '.$grps.' WHERE groupid <> 1 AND name IN ("'.$groupsini.'") ORDER BY name ASC';
//	var_dump($groupID);
